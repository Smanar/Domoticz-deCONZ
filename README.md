...
## Changelog.
- xx/xx/xx : 1.0.8 (On beta branch) > continuing new devices implementation. Adding a tool to delete all useless key created everytime launching Phoscon. Setting Websocket port is now useless.Change switches icon for button instead of bulb. Prevent the system be freezed if the gateway don't answer at a request and uising queue list for request to prevent collision.
